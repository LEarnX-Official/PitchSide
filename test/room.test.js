...
test('Room defaults to SwarmTransport when none is injected', () => {
  const room = new Room({ name: 'r', nickname: 'n', host: true, Hyperswarm, Corestore, crypto, b4a, storageDir: tmp() })
...
test('Room accepts an injected transport', () => {
  ...
  const room = new Room({ name: 'r', nickname: 'n', host: true, Corestore, crypto, b4a, storageDir: tmp(), transport: fakeTransport })
...
test('host can post events; guest post is a no-op (feed enforces host-only)', async () => {
  ...
  const host = new Room({ name: 'r', nickname: 'h', host: true, Corestore, crypto, b4a, storageDir: tmp(), transport: injected })
  const guestT = { onConnection () {}, onPeers () {}, async join () {}, async destroy () {} }
  const guest = new Room({ name: 'r', nickname: 'g', host: false, Corestore, crypto, b4a, storageDir: tmp(), transport: guestT })
...
test('events carry a seq, and host delete emits a tombstone targeting it', async () => {
  ...
  assert.ok(chats.every((e) => Number.isInteger(e.seq)), 'events carry seq')
...
test('guest cannot delete (host-only feed write rejected)', async () => {
  ...
  const guest = new Room({ name: 'r', nickname: 'g', host: false, Corestore, crypto, b4a, storageDir: tmp(), transport: t })
  await guest.open()
...
test('peer count callback fires through the transport', async () => {
  let emitPeers
  const t = { onConnection () {}, onPeers (fn) { emitPeers = fn }, async join () {}, async destroy () {} }
  const room = new Room({ name: 'r', nickname: 'n', host: true, Corestore, crypto, b4a, storageDir: tmp(), transport: t })
  let count = -1
  room.onPeerCount((n) => { count = n })
  await room.open()
  emitPeers(3)
