...
class Room {
  ...
  constructor ({ name, nickname, storageDir, host = false, Hyperswarm, DHT, net, Corestore, crypto, b4a, transport, mode = 'internet', localPort, remoteHost }) {
    this.name = name
  ...
  localBootstrapAddress () { return this.transport && this.transport.localBootstrapAddress }
  ...
  async open () {
    ...
    await this.feed.append({
      kind: 'system', author: this.nickname, at: Date.now(),
  ...
  postMatchEvent ({ type, minute, text }) {
    return this.feed.append({ kind: 'match', author: this.nickname, at: Date.now(), data: { type, minute, text } })
  }
  postChat (text) {
    return this.feed.append({ kind: 'chat', author: this.nickname, at: Date.now(), data: { text } })
  }
  postReaction (emoji) {
    return this.feed.append({ kind: 'reaction', author: this.nickname, at: Date.now(), data: { emoji } })
  }
  postCommentary (text, { persona } = {}) {
    return this.feed.append({ kind: 'commentary', author: 'AI', at: Date.now(), data: { text, persona } })
  }
  ...
  postDelete (targetSeq) {
    return this.feed.append({ kind: 'delete', author: this.nickname, at: Date.now(), data: { targetSeq } })
  }
  ...
  postBetHide (betId) {
    return this.feed.append({ kind: 'bet-hide', author: this.nickname, at: Date.now(), data: { betId } })
  }

  onEvent (fn) { return this.feed.onEvent(fn) }
  events () { return this.feed.events() }
